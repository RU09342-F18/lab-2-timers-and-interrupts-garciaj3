 ...
 * control by a button connected to P1.1 that managea the intensity of the LED connected to P2.0.
 *
...
int main(void) {
    ...
    P2OUT &= ~(BIT0); // LED OFF
    P2SEL0 |= BIT0; // P2SEL <- BIT0 must be 1 to create the pwm
    ...
    TB1CTL = TBSSEL__SMCLK | MC__UP; //Configuring the timer to use SMCLK and count in UP mode
